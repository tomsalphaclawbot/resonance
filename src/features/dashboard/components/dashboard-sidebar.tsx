...
function SelfHostFooter() {
  return (
    <SidebarFooter className="gap-3 py-3">
      <UsageContainer />
      <div className="group-data-[collapsible=icon]:hidden border border-border rounded-lg bg-background p-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <User className="size-4" />
          <span>Self-host local mode</span>
        </div>
      </div>
    </SidebarFooter>
  );
...
function SelfHostHeader() {
  return (
    <SidebarHeader className="flex flex-col gap-4 pt-4">
      ...
      <div className="group-data-[collapsible=icon]:hidden border border-border rounded-md bg-white px-3 py-2 text-xs text-muted-foreground">
        Auth disabled (self-host)
      </div>
    </SidebarHeader>
  );
}
...
function AuthSidebarShell({ pathname }: { pathname: string }) {
...
function SelfHostSidebarShell({ pathname }: { pathname: string }) {
  const [voiceDialogOpen, setVoiceDialogOpen] = useState(false);
  ...
  const othersMenuItems: MenuItem[] = [
    {
      title: "Settings",
      icon: Settings,
      onClick: () => window.alert("Organization settings are disabled in self-host mode."),
    },
    {
      title: "Local org",
      icon: Building2,
      onClick: () => window.alert("Using local org: selfhost-org"),
    },
    {
      title: "Help and support",
      url: "mailto:business@codewithantonio.com",
...
export function DashboardSidebar() {
  ...
  if (selfHostMode) {
    return <SelfHostSidebarShell pathname={pathname} />;
  }

  return <AuthSidebarShell pathname={pathname} />;
}
